...
let logoSrc=profile?.signature_logo_url||"";
if(logoSrc){try{const logoResponse=await fetch(logoSrc);if(logoResponse.ok){const logoBytes=new Uint8Array(await logoResponse.arrayBuffer());if(logoBytes.byteLength<=5242880){let logoBin="";for(let i=0;i<logoBytes.length;i+=32768)logoBin+=String.fromCharCode(...logoBytes.subarray(i,i+32768));const logoType=logoResponse.headers.get("content-type")||"image/png";attachments.push({filename:"ilchatsmail-logo."+(logoType.includes("jpeg")?"jpg":logoType.includes("gif")?"gif":logoType.includes("webp")?"webp":"png"),content:btoa(logoBin),content_type:logoType,content_id:"ilchatsmail-logo"});logoSrc="cid:ilchatsmail-logo"}}}catch(error){console.error("signature_logo_fetch_failed",error)}}
if(signature.name||logoSrc){signatureHtml='<div style="margin-top:24px;border-top:1px solid #ddd;padding-top:12px;font-family:Arial,sans-serif"><strong>'+esc(signature.name||"")+'</strong>'+(signature.role?'<div>'+esc(signature.role)+'</div>':"")+(signature.phone?'<div>'+esc(signature.phone)+'</div>':"")+(signature.site?'<div><a href="'+esc(signature.site)+'">'+esc(signature.site)+'</a></div>':"")+(logoSrc?'<div><img src="'+esc(logoSrc)+'" alt="Logotipo" style="max-width:240px;max-height:100px;margin-top:8px"></div>':"")+"</div>";}
...
const response=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:"Bearer "+key,"content-type":"application/json","Idempotency-Key":"ilchatsmail-"+m.id},body:JSON.stringify({from,to,cc:m.cc_emails||[],bcc:m.bcc_emails||[],reply_to:user.email,subject:m.subject||"(sem assunto)",text:textBody,html:htmlBody,attachments})});
